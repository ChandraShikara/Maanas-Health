...
app.post('/login', async (req, res) => {
  try {
    ...
    console.log(exist)
    jwt.sign(payload,/*key*/"student-jwt-key", { expiresIn: '1h' },
      (err, token) => {
        if (err) throw err
...
app.post('/tlogin', async (req, res) => {
  try {
    ...
    jwt.sign(payload,/*key*/"teacher-jwt-key", { expiresIn: '1h' },
      (err, token1) => {
        if (err) throw err

        const expiresIn = 3600; // 1 hour in seconds
        return res.json({ token1, expiresIn });
...
var transporter = nm.createTransport(
  {
    ...
    secure: false,
    auth: {
      user: 'mhproject19@gmail.com',
      pass: 'jeglqedryomocnwf'
    }
  }
);
